...
command -v medaka_consensus >/dev/null 2>&1 || { echo >&2 "Error: medaka not found."; exit 1; }
...
########################################
# Step 5: Run medaka to polish consensus sequences
########################################
echo "Step 5: Running medaka to polish consensus sequences..."

# Make directory
mkdir -p medaka_results
...
for i in {1..24}; do
    (
        ...
        BARCODE_ID="barcode${BARCODE_PADDED}"
        FASTQ_FILTERED="qc_reads/${SAMPLE_PREFIX}_barcode${BARCODE_PADDED}.filtered.fastq.gz"
        IRMA_CONSENSUS="irma_results/${BARCODE_ID}/amended_consensus/${BARCODE_ID}.fa"
        MEDAKA_OUTDIR="medaka_results/${BARCODE_ID}"
        
        if [ -f "$IRMA_CONSENSUS" ] && [ -f "$FASTQ_FILTERED" ]; then
            echo "Running medaka on ${BARCODE_ID}..."
            medaka_consensus -i "$FASTQ_FILTERED" -d "$IRMA_CONSENSUS" -o "$MEDAKA_OUTDIR" -t $THREADS -m r941_min_sup_g507
        fi
    ) &
    
    # Limit concurrent medaka jobs
    if [[ $(jobs -r -p | wc -l) -ge $THREADS ]]; then
        wait -n
    fi
done
wait  # Wait for all medaka jobs

step_complete "5" "Medaka polishing complete"

echo ""
echo "========================================"
echo "Output files: medaka_results/barcode01-24/consensus.fasta"
echo "========================================"
