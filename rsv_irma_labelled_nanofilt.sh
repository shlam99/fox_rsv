...
export SAMPLE_PREFIX BATCH THREADS MIN_LENGTH MIN_QUALITY MAX_LENGTH TARGET_BASES
...
command -v NanoFilt >/dev/null 2>&1 || { echo >&2 "Error: NanoFilt not found."; exit 1; }
...
########################################
# Step 1: Quality filtering with NanoFilt (parallelized)
########################################
echo "Step 1: Running NanoFilt in parallel (xargs)..."
...
seq 1 24 | xargs -P $THREADS -I {} bash -c '
    ...
    if [ -f "$FASTQ_IN" ]; then
        ...
        zcat "$FASTQ_IN" | \
        NanoFilt -q $MIN_QUALITY -l $MIN_LENGTH --maxlength $MAX_LENGTH --length $TARGET_BASES 2> "qc_reads/qc_logs/${SAMPLE_PREFIX}_barcode${BARCODE_PADDED}.nanofilt.log" | \
        gzip > "$FASTQ_FILTERED"
    else
